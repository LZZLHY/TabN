...
export async function register(req: Request, res: Response) {
  ...
  try {
    ...
    if (msg.includes('Unique constraint')) return fail(res, 409, '账号/邮箱/手机号/昵称已被占用', ErrorCode.ACCOUNT_OCCUPIED)
    return fail(res, 500, '注册失败', ErrorCode.INTERNAL_ERROR)
  }
}
...
export async function login(req: Request, res: Response) {
  ...
  if (!user) {
    ...
    return fail(res, 401, '账号或密码错误', ErrorCode.PASSWORD_INCORRECT)
  }
...
export async function me(req: AuthedRequest, res: Response) {
  ...
  if (!user) return fail(res, 401, '用户不存在', ErrorCode.USER_NOT_FOUND)
  return ok(res, { user })
}
