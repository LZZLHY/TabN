...
import { fail, ok, getErrorMessage } from '../utils/http'
...
export async function changeRootPassword(req: AuthedRequest, res: Response) {
  ...
  return ok(res, { ok: true, message: '密码修改成功，请重新登录' })
}
 ...
 */
export async function getTagStats(req: AuthedRequest, res: Response) {
  const actor = req.auth
  if (!actor) return fail(res, 401, '未登录')
  if (actor.role !== 'ADMIN' && actor.role !== 'ROOT') return fail(res, 403, '无权限')
  ...
  return ok(res, {
    ...
    totalUsage: tagStats.reduce((sum, t) => sum + t.count, 0),
  })
}

const UpdateBookmarkTagsAsAdminSchema = z.object({
 ...
 * Requirements: 3.2
 */
export async function updateBookmarkTagsAsAdmin(req: AuthedRequest, res: Response) {
  const actor = req.auth
  if (!actor) return fail(res, 401, '未登录')
  if (actor.role !== 'ADMIN' && actor.role !== 'ROOT') return fail(res, 403, '无权限')
  ...
  const parsed = UpdateBookmarkTagsAsAdminSchema.safeParse(req.body)
  if (!parsed.success) return fail(res, 400, parsed.error.issues[0]?.message ?? '参数错误')

  // 查找书签
  const item = await prisma.bookmark.findUnique({
    where: { id },
    select: { id: true, tags: true, user: { select: { id: true, role: true } } },
  })
  if (!item) return fail(res, 404, '书签不存在')

  // 权限检查：ADMIN 只能管理 USER 的书签
  if (actor.role === 'ADMIN' && item.user.role !== 'USER') {
    return fail(res, 403, '无权限（管理员只能管理普通用户的数据）')
  }

  // 验证并清理标签
  const { tags: sanitizedTags, validation } = validateAndSanitizeTags(parsed.data.tags)
  if (!validation.valid) {
    return fail(res, 400, validation.errors[0] ?? '标签格式错误')
  }
