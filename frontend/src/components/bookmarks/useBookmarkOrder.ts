...
type BookmarkItemWithTags = BookmarkItem & { tags?: string[] }
...
export function useBookmarkOrder(args: {
  ...
  // 关键：每次 itemIds 变化时都重新从 localStorage 读取，确保使用最新保存的顺序
  useEffect(() => {
    if (!userId) {
      setOrder(itemIds)
      return
    ...
    // 始终从 localStorage 读取最新的持久化顺序
    const persisted = getOrder(userId, folderId, context)
    const persistedSet = new Set(persisted)
    const itemIdsSet = new Set(itemIds)
    
    // 保留持久化顺序中仍然存在的 ID
    const filtered = persisted.filter((id) => itemIdsSet.has(id))
    // 添加新出现的 ID（不在持久化顺序中的）
    const extras = itemIds.filter((id) => !persistedSet.has(id))
