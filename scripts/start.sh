...
if [ ! -f "$ROOT_DIR/docker-compose.yml" ]; then
    ...
    echo ""
    echo "  请先运行安装脚本配置数据库："
    echo "  curl -fsSL https://raw.githubusercontent.com/LZZLHY/TabN/main/scripts/install.sh | bash"
    echo ""
    exit 1
fi
...
# 安装依赖并构建共享模块
echo -e "${BLUE}📦 步骤 3/4: 检查依赖${NC}"
echo "────────────────────────────────────────"

# 检查共享模块是否已构建
if [ ! -d "$ROOT_DIR/shared/dist" ]; then
    echo "构建共享模块..."
    cd "$ROOT_DIR" && npm install && npm run build:shared
    echo -e "${GREEN}✓ 共享模块已构建${NC}"
else
