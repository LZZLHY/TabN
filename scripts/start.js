...
async function startDatabase() {
  ...
  if (!fs.existsSync(dockerComposePath)) {
    ...
    log()
    info('请先运行安装脚本配置数据库:')
    info('  Windows: 双击运行 scripts\\install.bat')
    info('  Linux:   curl -fsSL https://raw.githubusercontent.com/LZZLHY/TabN/main/scripts/install.sh | bash')
    log()
    return false
  }
...
async function installDeps() {
  ...
  // 检查是否需要安装依赖（使用 workspaces，从根目录安装）
  if (!fs.existsSync(rootModules)) {
    info('安装项目依赖 (首次需要较长时间)...')
    try {
  ...
  if (!fs.existsSync(sharedDist)) {
    info('构建共享模块...')
    try {
      execSync('npm run build', { cwd: path.join(ROOT_DIR, 'shared'), stdio: 'inherit' })
      ok('共享模块构建完成')
    } catch {
      fail('共享模块构建失败')
      return false
